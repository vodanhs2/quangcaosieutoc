      ...
      <div class="card mb-4 p-4">
        ...
        <form action="" method="POST" role="form">
          ...
          <legend>Thêm Mới Giới Thiệu </legend>
           <div class="form-group">
            <select name="status" id="gioi-thieu" class="form-control" required="required">
              <option value="0" {{$about->status==0 ? 'selected' : ''}}>Giới thiệu Công Ty</option>
                <option value="1" {{$about->status==1 ? 'selected' : ''}}>Giới thiệu CEO</option>
                <option value="2" {{$about->status==2 ? 'selected' : ''}}>Giới thiệu Nhân Viên</option>
            </select>
           </div>
           <style>
             .nhanvien{
          ...
          <div class="form-group" >
            ...
            <input type="text" class="form-control  @if($errors->has('name'))is-invalid @endif" name="name" value="{{$about->name}}"  placeholder="nhập tên" >
            @if($errors->has('name'))
          ...
          <style>
            ...
            <div class="input-group">
              <input type="text" class="form-control @if($errors->has('image'))is-invalid @endif" name="image" id="image" placeholder="Trống" value="@if($about->image!=''){{url('uploads')}}/{{$about->image}}@endif" readonly>
               @if($errors->has('image'))
            <div class="invalid-feedback">
                        {{$errors->first('image')}}
                      </div>
            ...
            <img src="@if($about->image!=''){{url('uploads')}}/{{$about->image}}@endif" id="show-image" style="margin: 10px 0" class="img-fluid">
          </div>
          @if($about->status==0)
          <div class="form-group" id="iframe-youtube">
              <label for="">Mã nhúng youtube(nếu có)</label>
            <textarea name="video" class="form-control" value="{{$about->video}}" placeholder="nhập mã nhúng(iframe)"></textarea>
          </div>
          @endif
             <div class="form-group">
            <label for="">Nội dung(*)</label>
            <textarea name="content" class="ckeditor form-control @if($errors->has('content'))is-invalid @endif" placeholder="nhập nội dung">{{$about->content}}</textarea>
